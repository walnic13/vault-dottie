...
### 2.1 Dottie-L1 — personal, consensual 1:1 (`dottie_user_memory`)
...
- **Design change — add the Six-Plates lens** exactly as Theo's §7.5 did, but in Dottie's own table: a `plate text NULL` column (open, no CHECK; NULL = default **Work & Craft**; non-NULL = opt-in life-plate), + partial index. **INV-2/INV-5 hold:** this is Dottie's separate store, plate state stays L1-only, never crosses to Theo or to Dottie-L3/L4.
- **Consent:** Dottie-L1 is opt-in per individual (Amendment 9 "consensual"). A user may run Dottie with no personal memory at all (pure governance tool) or grant her a 1:1 memory.
...
- **O-DOTTIE-L1-CONSENT** — the consent + UX for a user granting Dottie a 1:1 memory (Amendment 9 "consensual") — default off? Per-surface?
