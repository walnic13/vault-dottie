...
2. Claude **v4 zip-deploys the sidecar** to `vaultgpt-func-dottie-stream` (whole-app `config-zip`, per the D2-Stream authority — NOT per-fn Kudu-VFS): rebuild the package `npm install` including **`jimp@0.22.12`** (pinned) alongside `pg`, deploy, restart.
