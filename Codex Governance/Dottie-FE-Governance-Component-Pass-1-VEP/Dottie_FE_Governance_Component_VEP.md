...
Grounding parent (source baseline): `1ecafccb6f382a392b517ccdb91532aef88b1c15` (vault-dottie, `development`)
...
| 6 | CHANGED — `src/theo/theme.ts` (adds `inset` + 8 semantic verdict tokens to `C`; §2.4/§2.1 values) | `Read` + `Edit` this turn | proposed `dd31705dc2e49961a2a319906bd4169d26568b1c` |
| 7 | CHANGED — `src/theo/lib/artifacts.ts` (`AssistantPart` gains `check`; `splitAssistant` + new `splitChecks` parse `[[CHECK]]…[[/CHECK]]`, suppress dangling opener) | `Read` + `Edit` this turn | proposed `65e4831aa9d7f298979b06429103a0989ed76859` |
| 8 | NEW — `src/theo/lib/check.ts` (`CheckData`/`Verdict`/`CheckSupport`/`CheckClaim`/`CheckConfidence` types + forgiving `parseCheck`) | `Write` this turn | proposed `f2ea6ae04423e64da1911e5d1ecc069531da9ff8` |
| 9 | NEW — `src/theo/components/GovernanceCheck.tsx` (the §3 anatomy component) | `Write` this turn | proposed `46a6ffa9a0904661019865590293d4bc5ec6508c` |
| 10 | CHANGED — `src/theo/components/TheoMain.tsx` (`renderAssistant` gains the `check` branch → `GovernanceCheck`, markdown fallback) | `Read` + `Edit` this turn | proposed `b3d65d3ec152738946deebb62cb5fb651c0883b9` |

No ChatGPT advisory cited. No backend / route / schema / migration touched (FE-only). The gold `DottieSpiral.tsx` (Logo-Mark VEP) and the pkg-1 dark palette are unchanged. `renderAssistant` is the single render seam — used directly for plain assistant turns and passed as `renderText` into `CitedText` (`ChatView.tsx:660`), so the `check` branch covers cited and non-cited answers alike.
...
- `splitAssistant` now splits each text segment on `[[CHECK]]` into interleaved `text` / `check` parts (after the existing artifact-sentinel split), so ordering with prose + artifacts is preserved.
- `renderAssistant` (`TheoMain`) gains a `check` branch: `parseCheck(body)` → `GovernanceCheck`; on any parse failure it falls back to `Formatted` (plain markdown), so a malformed block never blanks the turn.
...
- `tsc --noEmit -p tsconfig.app.json` → **exit 0** (this turn). New types (`CheckData` et al.), the `AssistantPart` `check` union member, and the `flatMap` return typing all resolve; no `any`.
- `npm run build` (`vite build`) → **clean** (461 modules transformed, built ~25s; `dist/` emitted). This turn.
...
| `artifacts.ts` `AssistantPart` / `splitAssistant` (`lib/artifacts.ts`) | `AssistantPart.kind` union gains `"check"` (value = raw JSON body); `splitAssistant(content: string): AssistantPart[]` signature **unchanged**; new module-private `splitChecks(segment: string): AssistantPart[]` | DOTTIE_DESIGN_SYSTEM §9 (renderer = markdown + governance blocks) | none — pure string parse |
| `TheoMain` `renderAssistant` (`components/TheoMain.tsx`) | **prop interface unchanged** (`TheoMainProps`); `renderAssistant(content)` gains an additive `part.kind === "check"` branch | DOTTIE_DESIGN_SYSTEM §9 | unchanged (same `t`/`mode` props) |
...
- **G-3 — Streaming + malformed-block behaviour.** A still-open `[[CHECK]]` (mid-stream, no closing tag yet) is suppressed — only the text before it renders, and the component pops in when `[[/CHECK]]` arrives (no raw-JSON flash). A permanently malformed/closed-but-unparseable block falls back to `Formatted` (renders the raw body as markdown text) rather than blanking the turn. Both are deliberate degradation paths. Disclosed.
...
All 5 files are git-diffable base→proposed at the grounding parent (GCR rows 6–10; blob SHAs listed).
...
- `lib/artifacts.ts` — `AssistantPart.kind` gains `"check"`; `splitAssistant` maps artifact-sentinel parts as before but routes text parts through new `splitChecks` (regex `\[\[CHECK\]\]\s*([\s\S]*?)\s*\[\[\/CHECK\]\]`), which interleaves `text`/`check` parts and suppresses a dangling unclosed opener.
- `lib/check.ts` — NEW: the `CheckData` type family + `parseCheck` (JSON.parse in try/catch; requires non-empty `lead`; normalises verdict to the 3 values or `null`; coerces claim/support/confidence/flags/docs to safe defaults; returns `null` on any untrusted shape).
...
- `components/TheoMain.tsx` — `renderAssistant` gains `if (part.kind === "check") { const data = parseCheck(part.value); return data ? <GovernanceCheck …/> : <Formatted …/> }`.
...
block (mirroring the deployed [[ARTIFACT]] protocol in lib/artifacts.ts) and draws GovernanceCheck; a malformed
or half-streamed block falls back to plain markdown (never blanks the turn). (2) Intensity is implicit in the
