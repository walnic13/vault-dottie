...
**Rev 4 — Codex Pass-2 REJECT remediation (T13, residual VEP prose).** Codex REJECTED rev 3 (`a166fd4`): §F-P2 still asserted "No prop-interface change to any existing component … (no signature change)," contradicting the CCT/GCR/DELTA which correctly disclose `ChatViewProps.renderAssistant` widening to `(content, streaming?)`. VEP-prose-only fix: §F-P2 now discloses the one additive, back-compatible prop widening (and that `TheoMainProps` is unchanged). Proactively scrubbed the remaining multi-rev staleness so no residual remains — the GCR invocation column now scopes each file to the rev that touched it (no false "this turn" on rev-1/rev-2 files), §DELTA notes `TheoMain.tsx` carries both rev-2 + rev-3, and §F-P6 marks tsc/build re-verified in rev 3. No source change.
...
Grounding parent (source baseline): `a166fd46a54985bc55ca2ccc6e462c71c636d18a` (vault-dottie, `development` — rev-3 HEAD Codex reviewed; this rev-4 VEP-prose-only remediation lands on top; no source change since rev 3)
...
| - | ------------------------------- | ------------------------------ | ---------------------------------------------------- |
| 1 | VISUAL AUTHORITY (binding) — `spec/DOTTIE_DESIGN_SYSTEM.md` (§2.4 semantic verdicts; §2.5 mono provenance; §3 component anatomy; §4 three intensities / R-INTENSITY; §5 anti-patterns; §9 R-RENDERER) | `Read` §44–121 (rev 1) + §181–204 (rev 2); binding, unchanged @ HEAD | `744523cf905df1186d954b86519b1cdeddac539c` |
...
| 6 | CHANGED — `src/theo/theme.ts` (adds `inset` + 8 semantic verdict tokens to `C`; §2.4/§2.1 values) | `Edit` rev 1; unchanged rev 2–3 | proposed `dd31705dc2e49961a2a319906bd4169d26568b1c` |
| 7 | CHANGED — `src/theo/lib/artifacts.ts` (`AssistantPart` gains `check`; `splitAssistant(content, streaming)` + new `splitChecks(segment, streaming)` parse `[[CHECK]]…[[/CHECK]]`; dangling opener suppressed only while streaming) | `Edit` rev 2; unchanged rev 3 | proposed `5b169ff910752683e7a03d31376692abb6df488a` |
| 8 | NEW — `src/theo/lib/check.ts` (`CheckData`/`Verdict`/`CheckSupport`/`CheckClaim`/`CheckConfidence` types + forgiving `parseCheck`; header comment corrected rev 2) | `Edit` rev 2; unchanged rev 3 | proposed `53fd892efab9541299efe2106c7ed3d3162fcb96` |
| 9 | NEW — `src/theo/components/GovernanceCheck.tsx` (the §3 anatomy component) | `Write` rev 1; unchanged rev 2–3 | proposed `46a6ffa9a0904661019865590293d4bc5ec6508c` |
| 10 | CHANGED — `src/theo/components/TheoMain.tsx` (`renderAssistant(content, streaming)` gains the `check` branch → `GovernanceCheck`, markdown fallback; rev-3 corrects the branch comment to say it always receives a fully-closed block) | `Read` + `Edit` rev 3 (comment-only); this turn | proposed `5774d7869deaee11f438809b2bc7d52725c8d1b2` |
| 11 | CHANGED — `src/theo/components/ChatView.tsx` (`renderAssistant` prop type widened to `(content, streaming?)`; direct call site passes `loading && i === messages.length - 1`) | `Read` + `Edit` rev 2; unchanged rev 3 | proposed `1e7b4898a8c44d9b3f96c6f2db213ef6560ef145` |
...
No new component is wired into any layout; `GovernanceCheck` is rendered only inside the assistant-turn body. The one existing prop-interface change is **additive and back-compatible** (rev-2): `ChatViewProps.renderAssistant` widened from `(content: string) => ReactNode` to `(content: string, streaming?: boolean) => ReactNode` (optional trailing param — every existing one-arg caller, including the `CitedText renderText` path, still type-checks and behaves identically). `TheoMainProps` is unchanged; the local `renderAssistant` / `splitAssistant` signatures gain the same optional trailing `streaming` param (default `false`). No other prop or component interface changes; the `AssistantPart.kind` union additively gains `"check"`. See §CCT for the full surface.
...
- `npm run build` (`vite build`) → **clean** (461 modules transformed; `dist/` emitted). Re-verified rev 3, this turn.
...
All 6 files are git-diffable base→proposed (GCR rows 6–11; blob SHAs listed). theme.ts + GovernanceCheck.tsx are the rev-1 blobs (unchanged since); artifacts.ts / check.ts / ChatView.tsx carry the rev-2 change (unchanged rev 3); TheoMain.tsx carries the rev-2 (streaming param) + rev-3 (comment-only) change.
