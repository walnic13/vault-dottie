...
Codex Pass-2 **APPROVED** (rev-2). Walter ran `dottie_attachments_migration.sql` as `pgadmin_vault` against `vaultgpt-postgres-prod`. Claude read-only-verified (catalog, RO role `codex_reporting_ro`): **12 columns** (correct types/nullability); FK `dottie_attachments_conversation_id_fkey` → `dottie_conversations(id)` **ON DELETE SET NULL** (via `pg_constraint`, `confdeltype='n'`); RLS enabled + four `_own` policies (select/insert/update/delete); helper `dottie_attachment_exists_unscoped` `prosecdef=t`, `search_path=public`, ACL `pgadmin_vault=X/… authenticated=X/…` — **no PUBLIC** entry (the rev-2 REVOKE-PUBLIC hardening confirmed live); indexes pkey + `_created_by` + `_conversation`; CHECKs `byte_size >= 0` + `filename` non-empty. Role-C: schema doc `spec/DOTTIE_AZURE_POSTGRES_SCHEMA.md` §3 + new §5 record the deployed table. The verify SQL's FK query was switched to `pg_constraint` (the `information_schema.*_column_usage` form is invisible to the RO role). The paired **Attachments-Handlers** package is now unblocked at the schema gate (still pending its own Codex approval).
