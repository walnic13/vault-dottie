...
**F-P3 Backend/contract grounding.** The gateway (`gateway.live.ts`) is repointed from Theo's `theo_*` endpoints to Dottie's deployed `dottie_*` contract (`dottie_message_stream`, `dottie_message`, `dottie_list_conversations`, `dottie_get_conversation` — recorded in `spec/DOTTIE_API_SPEC.md`), and its stream parser gains an OpenAI-chunk branch (`choices[0].delta.content`) alongside the Anthropic branch, because Dottie's backend emits Azure OpenAI-shape SSE (gpt-5), not Anthropic events. Endpoints Dottie's backend does not yet serve (attachments/projects/artifacts/voice) remain on their `theo_*` names and are disclosed as gaps (G-3).
...
- **G-3 (unbacked endpoints — the COMPLETE audit): PRE-LAND (governed backend packages + FE gating).** The FE calls ~35 endpoints; Dottie's backend serves only the 4 core-chat ones. The full endpoint-by-endpoint reconciliation is `spec/DOTTIE_THEO_RECONCILIATION.md` (authoritative gap register). **Errors today** (not merely absent): `dottie`-missing `theo_delete_conversation` (delete chat — user-hit), `theo_rename_conversation`, `theo_set_conversation_starred`, `theo_list_people`, attachments (5), artifacts-persist (3), voice (2), and all Projects endpoints (14, pending the scope decision). **Remediation:** (a) the FE controls for missing features are GATED/HIDDEN until their backend lands (so nothing errors) — a fast-follow FE package; (b) each missing group becomes a governed backend package (`dottie_*` mirroring the Theo original), sequenced conversation-management first. Until then, Dottie does NOT match Theo beyond core chat — stated plainly, not represented otherwise.
