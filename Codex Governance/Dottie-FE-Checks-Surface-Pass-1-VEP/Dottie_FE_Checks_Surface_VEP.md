...
The second FE increment of the 9/10 governance console (DOTTIE_DESIGN_SYSTEM §6.1). Adds the dedicated **Checks on Theo** surface — the full, verdict-filterable log of every check Dottie has made (richer than the Overview's recent-checks strip: each card shows the conclusion + documentation-expected, and clicks through to the source turn) — and **traceability** (clicking a finding opens the conversation where Dottie made it, via the existing `selectRecent`). Reads the same DEPLOYED `dottie_findings_list` handler as the Overview (pkg 3a.2); no new backend. To share the card vocabulary it extracts the finding/flag render primitives from `OverviewView` (pkg 3b.1) into a shared `FindingCard.tsx` — a **rendering-preserving extraction** (same JSX/styles). **One additive Overview behaviour change (disclosed):** the Overview now also passes `onOpen`, so its finding cards become clickable (open the source turn), matching the Checks cards — new in 3b.2, not a pure extract. **Additive + FE-only:** a new `ChecksView` + shared `FindingCard` + the `View` union + one nav entry; no change to the chat, the pkg-2 renderer, the backend, or any route/schema. Open-flags / Audit surfaces + Overview-as-default-landing are pkg 3b.3; Workflows / Library (need `dottie_review_chains`) are pkg 3c.
...
| governance/CLAUDE_CODE_THEO_FRONTEND_GOVERNOR_STANDARD.md | §4 | "do not redesign" | §UI-RECON — additive; the OverviewView change is a rendering-preserving extract + one additive click-through behaviour (disclosed §F-P2/§GAP G-4) |
...
- Extract: the finding/flag render primitives move from `OverviewView` (3b.1) into a shared `components/FindingCard.tsx`; `OverviewView` now imports them (**rendering unchanged** — same JSX/styles) and threads `onOpen` (which makes its finding cards clickable — the additive Overview behaviour above). `FindingCard` gains two optional props: `onOpen?` (click-through when the finding links a conversation) and `detail?` (show conclusion + docs, used by Checks; Overview omits for density).
...
The `OverviewView` diff is the rendering-preserving extraction (same JSX/styles) **plus one additive behaviour**: it now passes `onOpen={t.selectRecent}` (via `TheoMain`), so its finding cards become clickable to open the source turn — a new traceability behaviour on the Overview cards (they were static in 3b.1). No prop-interface change to `OverviewView`; no other existing behaviour/prop/DOM change beyond the additive `checks` branch.
...
`ChecksView` **implements** the binding §6.1 "Checks on Theo" section; the `FindingCard` extract preserves the APPROVED 3b.1 card rendering (gaining the optional `onOpen`/`detail` props), and the Overview additionally opts into the click-through (disclosed §F-P2/§GAP G-4). Verdict colours are the semantic `C` tokens (§2.4 — badges, filter tabs, card rules, callout; never the gold accent, which appears only on the neutral "All" active tab); provenance is `MONO` (§2.5). Purely additive — no existing view, the chat, the renderer, or the backend is touched (§4 "do not redesign"). No VISUAL-AUTHORITY-DEVIATION rows.
...
| `OverviewView` (`components/OverviewView.tsx`) | **props unchanged** (`OverviewViewProps`); refactored to import `FindingCard`/`FlagRow`/`MICRO` from the shared module (rendering unchanged) + now passes `onOpen={onOpenConversation}` — **an additive behaviour: its finding cards become clickable** (open the source turn), where they were static in 3b.1 | DOTTIE_DESIGN_SYSTEM §6.1 | unchanged |
...
- **G-4 — Click-through reuses `selectRecent`; the Overview cards gain it too.** Opening a finding's conversation switches to Ask Dottie with that thread (via the deployed `dottie_get_conversation`); a finding with a null/`conversation:`-synthetic `conversation_id` is non-clickable. This behaviour is wired into **both** console views — so the **Overview** finding cards (static in 3b.1) also become clickable this package. Additive, non-destructive; the APPROVED 3b.1 Overview rendering is unchanged. Disclosed.
...
All 8 files git-diffable base→proposed (GCR rows 7–14). NEW: `FindingCard.tsx`, `ChecksView.tsx`. CHANGED (all additive/refactor): `OverviewView.tsx` (imports the shared primitives instead of local copies — rendering unchanged — **and passes `onOpen=selectRecent`, an additive click-through behaviour making its finding cards clickable**), `types.ts` (`View` += checks), `TheoMain.tsx` (+import +label +render branch +onOpen wiring), `useTheoState.ts` (applyView/currentLoc checks + comment), `data.ts` (+checks NavItem + comment), `icons.tsx` (+IcChecks). No existing function/prop/DOM of any non-console view changed; no backend.
...
FindingCard extracted from the APPROVED 3b.1 OverviewView (rendering-preserving extraction: OverviewView base 9bff424c ->
proposed 0e8ec22a imports the shared primitives, rendering unchanged; ONE ADDITIVE BEHAVIOUR disclosed — Overview now passes
onOpen=selectRecent so its finding cards become clickable, static in 3b.1, §GAP G-4) + the View union += checks +
