...
| 3 | CHANGED + AUTHORITY — FE Governor — `c:/Users/WalterMansfield/Vault Group LLP/Innovate - Documents/Tax Workpapers Project/2026/vault-dottie/governance/CLAUDE_CODE_THEO_FRONTEND_GOVERNOR_STANDARD.md` (§3 browser-storage rule — **amended to add the Walter-authorized Dottie Snapshot Storage Exception**, corrected to match the implemented behaviour: per-principal purge + isolation by exclusive `vault-dottie:v1:` keying, NO foreign-app purge; this is the storage authority for row 5) | `Read`(§37) + `Edit` this turn | base @ parent (pre-exception `3afec7ea4b13650ce2bf28bf32073179a35e7b24`) → proposed @ review HEAD `b6ef105fea53533f45d0e907da223616a61c51dd` |
...
| 5 | CHANGED — `c:/Users/WalterMansfield/Vault Group LLP/Innovate - Documents/Tax Workpapers Project/2026/vault-dottie/src/theo/services/theoSnapshot.ts` (the localStorage snapshot cache; `PREFIX` re-namespaced @ 58e8282; **comments corrected this turn — the file header + PREFIX block now name the Dottie exception / `dottie_*` loaders and state explicitly that the foreign-purge spans ONLY Dottie's own namespace, never Theo's keys; T13 fix**) | `Read`(full) + `Edit` this turn | base @ parent `f41806e` `d653da7632135a09d71c26c6c5f077060aca74dc` → proposed @ review HEAD `1c7ecbf77ca6e415e43f7cd946c2560b748d0fde` |
...
| c:/Users/WalterMansfield/Vault Group LLP/Innovate - Documents/Tax Workpapers Project/2026/vault-dottie/governance/CLAUDE_CODE_THEO_FRONTEND_GOVERNOR_STANDARD.md | §3 | "namespaced per authenticated principal `vault-dottie:v1:<oid>:*`" | §1(1) — the `PREFIX` re-namespace realizes the authorized Dottie namespace + its foreign-PRINCIPAL purge (which spans that namespace only; never Theo's `vault-theo:v1:` keys) |
...
- `tsc --noEmit -p tsconfig.app.json` → **exit 0** this turn (the only source change since `58e8282` is the theoSnapshot.ts comment correction — no behaviour change; the governor amendment is docs-only).
- `npm run build` (`vite build`) → **clean**; the federated `__federation_expose_DottieSurface` chunk emits.
- Behavioural: after deploy, Dottie reads her empty `vault-dottie:v1:<oid>:*` namespace (no Theo bleed); her `bindPrincipal` purge spans only her own namespace (foreign-PRINCIPAL keys) and never touches Theo's `vault-theo:v1:` keys; the 9/10 panel renders on Dottie's `#0C0F14` ground. Best eyeballed on the dev SWA mounted in Origin (§GAP).
...
- **G-4 — Existing shared-namespace cache is abandoned, not migrated.** After deploy Dottie reads the fresh `vault-dottie:v1:` namespace and purges only foreign-principal keys within it; any pre-existing `vault-theo:v1:` entries are Theo's and are left untouched. No migration needed (revalidated seed). Disclosed. PROCEED.

## §DELTA — changed files (before → after evidence)
Three files. **`governance/CLAUDE_CODE_THEO_FRONTEND_GOVERNOR_STANDARD.md`** (`3afec7ea`→`b6ef105f`): §3 appends the Walter-authorized Dottie Snapshot Storage Exception clause (Dottie surface, `vault-dottie:v1:<oid>:*`, foreign-PRINCIPAL purge spanning only Dottie's own namespace, isolation from Theo by exclusive `vault-dottie:v1:` keying — NO foreign-app purge of `vault-theo:v1:` — same BINDING constraints); the closing sentence now reads "any surface not named in these two exceptions (Theo, Dottie) … prohibited". **`theoSnapshot.ts`** (`d653da76`→`1c7ecbf7`): `PREFIX` `'vault-theo:v1:'`→`'vault-dottie:v1:'` (@ 58e8282) + the header/PREFIX comments corrected this turn to name the Dottie exception + `dottie_*` loaders and state the foreign-purge spans only Dottie's namespace (never Theo's keys). **`TheoMain.tsx`** (`0150b85d`→`fdd5c4d1`, unchanged this turn): this package added `background: C.bg` to the root style (the intermediate blob at commit `58e8282` was `c420af97`); the current blob `fdd5c4d1` additionally carries `color: C.ink` from the separately-APPROVED text-color package `5b6e944` — not claimed here. No other bytes changed.

## §CODEX — activation (Walter forwards)

```
Codex is activated for Pass-2 FRONTEND review of the Dottie Mount Companion Corrections RE-ISSUE #2, vault-dottie,
"Codex Governance/Dottie-Mount-Companion-Corrections-Pass-1-VEP/Dottie_Mount_Companion_Corrections_VEP.md" @ commit <HEAD>.
Open Pass-2 with a governance-bound GCR + Rule Anchor Table; hard-gate; emit only APPROVED or REJECTED. Re-issue #2 addressing
the two REJECT gates on a508117. Scope: two FE source files + one governance-doc amendment; no backend/schema/route. FIX A
(storage authority now matches code): the prior exception draft required a foreign-app purge of vault-theo:v1: keys, but
bindPrincipal() only purges within PREFIX=vault-dottie:v1: (foreign-PRINCIPAL) — and Dottie must NOT delete Theo's keys
(that is the original bug in reverse). Corrected: FE Governor §3 Dottie Snapshot Storage Exception now states isolation is by
keying every read/write/purge EXCLUSIVELY under vault-dottie:v1: (Dottie never reads/writes/purges vault-theo:v1:), and the
on-mount purge is a foreign-PRINCIPAL purge spanning ONLY Dottie's namespace (drops vault-dottie:v1:<otherOid>:*) — matching
the code exactly. Governor 3afec7ea->b6ef105f. FIX B (T13 source/prose): theoSnapshot.ts header + PREFIX comments still named
the Theo exception / theo_* loaders — corrected to the Dottie exception (2026-08-05) + dottie_list_conversations /
dottie_get_conversation / dottie_list_people, and now state the foreign-purge spans only Dottie's namespace (never Theo's keys).
theoSnapshot d653da76->1c7ecbf7. Carried from re-issue #1 (both APPROVED-clean there): GCR currency labels base @ parent ->
proposed @ review-HEAD (anchored to blob SHAs). The two functional edits stand: (1) theoSnapshot.ts PREFIX
'vault-theo:v1:'->'vault-dottie:v1:' isolates Dottie's cold-open cache from Theo (shared Origin-page localStorage), authorized
by the exception; (2) TheoMain root paints background: C.bg (#0C0F14, DOTTIE §2.1) so mode="panel" fills the 9/10 dark (its
sibling color: C.ink is the separately-APPROVED text-color pkg 5b6e944 — attribution split in the currency row + §DELTA). tsc
-p tsconfig.app.json exit 0 + vite build clean. Mechanical lint PASS. Emit APPROVED or REJECTED only.
```
