...
## Repair note (rev-2 — addresses Codex REJECT T13 / T20 / T22)
Codex found a **fifth** attachment path left un-gated: `listConversationAttachments()` fires on **every chat reopen** (`useTheoState.selectRecent` L614, unconditional — a READ independent of the paperclip control), so it hit `/api/theo_list_conversation_attachments` on normal chat open (caught/non-fatal, but a live unbacked call — contradicting the "removes premature calls" posture). Fixed: added `if (!DOTTIE_CAPABILITIES.attachments) return [];` to `listConversationAttachments()` (same `[]` idiom as the others). CCT / F-P3 / §UI-RECON updated to list this fifth path. Whole-file re-sweep of every `theo_*` call vs. its trigger confirms this was the only unbacked call on a NORMAL path (mount/reopen/send): the project-scoped reads fire only via the hidden Projects view or a `project_id`-guarded restore (L609 — no Dottie conversation can carry a `project_id`), and attachment-write/voice/gallery/add-to-project calls sit behind now-hidden controls. `tsc` + `npm run build` clean.
...
**F-P3 Backend/contract grounding.** No new backend contract. This package REMOVES premature calls to endpoints Dottie does not serve: `theo_list_projects` (projectsBase), `theo_list_people`, `theo_list_artifacts` / `theo_upsert_artifact`, and **`theo_list_conversation_attachments`** (apiBase) — each short-circuited in `gateway.live.ts` to a benign empty/no-op (the same `[]`-return pattern already approved for `listProjectConversations`), and the `attachmentsAvailable()` / `voiceAvailable()` predicates now return `false` (they were `isLive()` = `true` in the Origin mount, which is why the paperclip + mic showed and would error). The `listConversationAttachments` short-circuit is load-bearing: that READ fires on **every chat reopen** (`selectRecent`), independent of the paperclip, so gating the predicate alone would leave it calling an unbacked endpoint on normal chat open. Whole-file sweep confirms no other `theo_*` call fires on a normal (mount/reopen/send) path — the rest sit behind the hidden Projects view, a `project_id`-guarded restore, or now-hidden controls. The live gateway's 7 served `dottie_*` routes are unaffected.
...
| Attachments (compose) | `ChatView.tsx` paperclip/paste/drag-drop (all honor `attachmentsAvailable`) | predicate → `false` | AUTHORIZED-DELTA |
| Attachments (reopen READ) | `useTheoState.selectRecent` L614 → `listConversationAttachments` (fires on every chat open, not behind a control) | `listConversationAttachments` short-circuits to `[]` on `!attachments` | AUTHORIZED-DELTA (rev-2 — the fifth path Codex flagged) |
...
// listConversationAttachments(): if (!DOTTIE_CAPABILITIES.attachments) return [];   // rev-2: fires on every reopen
