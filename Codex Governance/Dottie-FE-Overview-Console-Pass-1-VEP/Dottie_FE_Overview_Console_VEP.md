...
| 11 | CHANGED — `src/theo/useTheoState.ts` (`findings`/`flags`/`overviewLoading` state + `loadOverview` + `applyView("overview")` + return-object expose) | `Read` + `Edit` this turn | proposed `8e6ca3cd5e3ddef70d680d2d91fc753b5b2ecc18` |
...
| 13 | CHANGED — `src/theo/data.ts` (`NAV` gains the `overview` entry, gated on `DOTTIE_CAPABILITIES.overview`; `chats` relabelled "Ask Dottie") | `Read` + `Edit` this turn | proposed `190cbb58fdab7fe27eb88253f46ea12013911a0e` |
| 14 | CHANGED — `src/theo/swapBlock.ts` (`DOTTIE_CAPABILITIES.overview = true` — the read handlers are live) | `Read` + `Edit` this turn | proposed `6fdf735d83c1f86469c5e859b420865af6d1f126` |
