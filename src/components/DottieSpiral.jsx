...
// DottieSpiral — Dottie's mark. The Vault logo (Spiral of Theodorus) + a continued nautilus tail, drawn
// as a full shell — but where Theo's mark BUILDS seed→out, Dottie's DECONSTRUCTS outer→seed, wedge by
// wedge, down to a single dot at the spiral's origin (Dottie is named after the Dottie number — the fixed
// point at the centre), holds on the dot, then rebuilds. Static / reduced-motion = the full shell + dot.
//
// The wedge geometry is byte-verbatim from vault-origin/public/icon.svg (via vault-theo SpiralAssemble):
// the logo stays exact; only the tail is synthesised (extra Spiral-of-Theodorus triangles from the real
// outer edge). Reveal order (data-o) = seed→out, so DECONSTRUCTING = hiding in descending data-o.
...
// --- recovered logo geometry (shared apex O, unit U, ~17 logo triangles + tail) ---
const O = { x: 88.94, y: 68.06 };
const U = 108.84 / Math.sqrt(17);
const TH17 = (85.7 * Math.PI) / 180;
const N = 32; // logo (~18) + continued tail

const LOGO_PATHS = WEDGES_SVG.match(/<path[^>]*\/>/g) ?? [];
const N_LOGO = LOGO_PATHS.length;

function tailPoints(n) {
  const P = {};
  let th = TH17;
...
// persistent centre dot at the spiral's origin O — Dottie never fully vanishes; the dot is the fixed point.
const DOT_MARKUP = `<circle class="dot" cx="${O.x}" cy="${O.y}" r="3.4" fill="#991b1e"/>`;

/**
 * DottieSpiral — static full shell + centre dot; when `animate`, deconstructs outer→seed to the dot,
 * holds, rebuilds seed→out, forever. Respects prefers-reduced-motion (holds the full shell).
 */
export function DottieSpiral({ size = 28, animate = false, title = "Dottie" }) {
  const ref = useRef(null);
  useEffect(() => {
    const host = ref.current;
    if (!host || !animate) return;
    const ws = Array.from(host.querySelectorAll(".w")).sort(
    ...
    const STEP = 90, HOLD_DOT = 480, HOLD_FULL = 620, n = ws.length;
    ws.forEach((w) => { w.style.transition = "opacity 0.20s ease-out"; w.style.opacity = "1"; });
    let timers = [];
    const run = () => {
      timers.forEach((t) => window.clearTimeout(t));
      timers = [];
      // DECONSTRUCT outer→seed: hide in descending reveal-order (outermost first)
      ws.forEach((w, i) => { const back = n - 1 - i; timers.push(window.setTimeout(() => { w.style.opacity = "0"; }, back * STEP)); });
      const gone = n * STEP;
      // REBUILD seed→out after holding on the dot
      ws.forEach((w, i) => timers.push(window.setTimeout(() => { w.style.opacity = "1"; }, gone + HOLD_DOT + i * STEP)));
      timers.push(window.setTimeout(run, gone + HOLD_DOT + n * STEP + HOLD_FULL));
    };
    run();
    return () => { timers.forEach((t) => window.clearTimeout(t)); };
  }, [animate]);
  const svg = `<svg width="${size}" height="${size}" viewBox="${VB}" preserveAspectRatio="xMidYMid meet"><g>${LOGO_MARKUP}${TAIL_MARKUP}${DOT_MARKUP}</g></svg>`;
  return (
    <span
      ref={ref}
      role="img"
