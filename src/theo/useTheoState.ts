...
export function useTheoState(launchAppContext?: AppContext) {
  ...
  // Default is contextual ONLY at launch: app-aware iff the LAUNCH context (passed at mount) had an
  // app_key, general otherwise. Thereafter the mode changes ONLY via the user's mode chip (setAgentMode)
  // — a later appContext update from the host MUST NOT flip it (App Host §6D(3): context availability ≠
  // mode). General mode ignores the published context entirely (effectiveAppContext → EMPTY_APP_CONTEXT).
  ...
  // §6D(3): the effective context. General mode substitutes EMPTY_APP_CONTEXT so ALL downstream review
  // logic (persona, per-review project, review agent, landing) behaves exactly as a context-free launch.
  ...
  return {
    ...
    // §6D(3): explicit user switch of the right-panel agent between app-aware and general (the mode chip)
    // — the ONLY thing that changes the launch-latched mode. Starts a fresh chat so the new mode lands
    // cleanly (app-aware re-arms the review chat; general shows a fresh general chat).
