...
3. **No browser storage — scoped exception.** No `localStorage` / `sessionStorage`; 1A state is React/in-memory. Persistence is 1B. **Exception — Theo Snapshot Storage Exception (Walter-authorized 2026-07-29):** the Theo frontend (`vault-theo` `src/theo/`) MAY use `sessionStorage` or `localStorage` to persist an instant-paint snapshot consisting ONLY of (a) the recents list, (b) the last-opened conversation's first page, and (c) the self-identity row (the greeting name/id). Constraints (all BINDING): metadata + first-page + self-identity ONLY — never access tokens, secrets, credentials, model keys, or conversation history beyond the first page; namespaced per authenticated principal (Entra OID: `vault-theo:v1:<oid>:*`); because `localStorage` survives session end and Theo has no logout seam, at-rest isolation is the per-principal namespace PLUS an active purge of foreign-principal namespaces on mount; employees-only origin; ALWAYS paired with revalidation (an instant-paint seed, never authoritative-stale — the governed `theo_list_conversations` / `theo_get_conversation` / `listPeople` loaders always fetch fresh and overwrite; a revalidation-preserving seed is NOT a snapshot lane bypassing the authority regime per the Architecture "Singular audit story"); all writes try/guarded. **Exception — Dottie Snapshot Storage Exception (Walter-authorized 2026-08-05):** the Dottie frontend (`vault-dottie` `src/theo/`), as the governed Theo replica, MAY use `sessionStorage` or `localStorage` to persist the SAME instant-paint snapshot — ONLY (a) the recents list, (b) the last-opened conversation's first page, and (c) the self-identity row (the greeting name/id) — under the SAME BINDING constraints as the Theo exception above, with these Dottie specifics: namespaced per authenticated principal `vault-dottie:v1:<oid>:*`; because Dottie has no logout seam, at-rest isolation is the per-principal namespace PLUS an active purge of foreign-principal namespaces on mount (the same foreign-PRINCIPAL purge as the Theo exception, spanning ONLY Dottie's own `vault-dottie:v1:` namespace); isolation from the co-hosted Theo agent — which shares the one Origin-page `localStorage` — is achieved by keying every read/write/purge EXCLUSIVELY under `vault-dottie:v1:`: Dottie never reads, writes, or purges Theo's `vault-theo:v1:` keys (and must not); ALWAYS paired with revalidation via the governed `dottie_*` loaders (which always fetch fresh and overwrite; a revalidation-preserving seed is NOT a snapshot lane bypassing the authority regime). All other browser storage — any surface not named in these two exceptions (Theo, Dottie) — remains prohibited under this rule.
